...
def test_c(model, base_path):
    ...
    for cname in CIFAR10C.CORRUPTIONS:
        ...
        logger.info(f"{cname}: Loss: {metrics['test_loss'].avg:.4f}, Acc: {metrics['test_acc'].percent:.4f}")

        corruption_metrics[cname] = metrics

    avg_acc = np.mean([metrics['test_acc'].percent for metrics in corruption_metrics.values()])
    logger.info(f"Average Accuracy: {avg_acc:.4f}")

    return corruption_metrics[cname]
